...
int run_test_nil(Testcase test) {
    ...
    for (int i = 0; i < test.number; i++) {
        ...
        } else if (strcmp(action, "testLru") == 0) {
            //print_kernel_free_space(kernel);
            char str_buf[128], ch;
            fscanf(file, "%s", str_buf);
            while ((ch = getc(file)) != '#')
                strncat(str_buf, &ch, 1);
            // strncat(str_buf, "\n", 1);
            get_kernel_lru_info(kernel, buf);
            if (strcmp(buf, str_buf) == 0) {
                score += 1;
                //printf("Action %d Correct\n", i);
            } else {
    ...
    for (int i = 0; i < test.number; i++) {
        ...
        if (strcmp(action, "create") == 0) {
            ...
            if (pid == answer) {
                score += 1;
                //printf("Action %d Correct\n", i);
                printf("Score granted! now %d\n\n", score);
            } else {
                //printf("Action %d Wrong\n", i);
                printf("Return %d not match! now %d\n\n", pid, score);
            }
...
int main(int agrc, char *argv[]) {
    ...
    if (select == 10) {
        ...
        run_test(testcases[select]);
    } else {
        printf("Please type a number of '0' ~ '10'!\n");
        return 0;
    }
}